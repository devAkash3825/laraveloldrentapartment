...
@section('title', 'RentApartements | Listing Properties ')
@push('style')
@endpush
@section('content')
<!-- Premium Header -->
        ...
        <div class="row">
            ...
            <div class="col-lg-12">
                <div class="d-flex justify-content-between">
                    <ul class="nav nav-pills d-inline-flex justify-content-end mb-2">
                        <li class="nav-item me-0">
                            <form class="quicksearchform" method="get" action="{{ route('search-property') }}">
                                <div>
                                    <div class="listting_filter-top">
                    ...
                    <ul class="nav nav-pills d-inline-flex justify-content-end mb-2 gap-3">
                        <li class="nav-item me-0">
                            <form id="filterForm" method="get" action="{{ route('search-property') }}">
                                <div>
                                    <div class="sidebar_line_select">
                                        <select class="select_2" name="date_filter" id="date_filter">
                                            <option value="All"
                                                {{ request('date_filter') == 'All' ? 'selected' : '' }}>Recent
                                                Properties</option>
                                            <option value="last_month"
                                                {{ request('date_filter') == 'last_month' ? 'selected' : '' }}>Last
                                                Month</option>
                                            <option value="custom_range"
                                                {{ request('date_filter') == 'custom_range' ? 'selected' : '' }}>Custom
                                                Date Range</option>
                                        </select>
                                    </div>


                                    <div id="customDateRange" class="sidebar_line_select" style="display: none;">
                        ...
                        <li class="nav-item me-0">
                            <a class="user_btn btn-main" href="{{ route('add-property') }}"><i
                                    class="far fa-plus" aria-hidden="true"></i> Add Property</a>
                        </li>
                        @endif
                        @endif
                    </ul>
                ...
                <div id="propertyGrid" class="row g-4">
                    @include('partials.property_list', ['propertyDetails' => $propertyDetails])
                </div>


                <div id="notFoundMessage" class="text-center my-4" style="display:none;height:50vh;">
                ...
                <div id="pagination" class="d-flex justify-content-center pagination">
                    @include('partials.pagination_links', ['paginator' => $propertyDetails])
                </div>

                {{-- <div id="pagination" class="d-flex justify-content-center pagination">
                        @include('partials.search_pagination', ['paginator' => $propertyDetails])
                    </div> --}}
